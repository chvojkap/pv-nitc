...
\begin{remark} We note that our definition of the decommitment oracle $\deco$ slightly differs from the original definition in \cite{TCC:KatLosXu20}, since we require that oracle at first checks if the commitment is well formed and only then returns the result of $\fdecom$. All our constructions can achieve also original definition, since it is sufficient to include the proof $\pi$ that commitment is well formed as a part of the commitment and then directly include the check if a commitment is well formed in the description of $\fdecom$. However, in that case $\pi_\com$ would be empty and the whole idea of the separation of a commitment form a proof would be meaningless. This is also one of the reasons, why the original paper requires several NIZK proofs in order to achieve tight reductions, since in \cite{TCC:KatLosXu20} $\fdecom$ checks well-formedness under the hood in decryption algorithm of timed-encryption algorithm. There are several ways how to avoid this issue, if one wants to keep commitment and proof separated: 1. one can change the syntax of $\fdecom$ algorithm and include $\pi_\com$ as a part of the input parameters or 2. one can adjust the definition of $\deco$ oracle, as we did. We prefer the second solution, since from the correctness definition and $\mathsf{BND \mhyphen CCA}$ definition of NITC is clear that we care about $\fdecom$ only if $\cvrfy$ algorithm is executed. Therefore we believe this should be also the case for $\deco$ oracle.
