...
\begin{enumerate}
\item $\prv$ samples $\alpha \rand \smplset$, computes $a_0:=g^\alpha, a_1:= (h_1\cdot h_2^{-1})^\alpha, a_1:= (h_3\cdot h_2^{-1})^\alpha$ and sends $(a_0, a_1, a_2)$ to $\vrf$.
...
\item $(h_3\cdot h_2^{-1})^z = (h_3\cdot h_2^{-1})^{\alpha + v \cdot r} = (h_3\cdot h_2^{-1})^\alpha \cdot (h_3^r\cdot (h_2^r)^{-1})^v = a_2 \cdot (h_3^r \cdot m \cdot (h_2^r \cdot m)^{-1})^v = a_2 \cdot (c_1\cdot c_2^{-1})^v$;
