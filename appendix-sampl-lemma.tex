...
We construct an adversary $\advB$ that breaks soundness with respect to auxiliary input $\aux:=(p,q)$ of the NIZK. W.l.o.g we assume that whenever event $\event$ happens, $\adv$ previously asked the random oracle $H$ on input $(h_1,h2,c_0,c_1,c_2,a_0,a_1)$. Let $\adv$ makes at most $Q$ random oracle queries. 
The adversary $\advB_{\secpar}(p,q)$ proceeds as follows:
\vspace{-2mm}
\begin{enumerate}
\item Samples $k_1\rand \smplsetqrn$, computes $h_1 := g^{k_1} \bmod N,\varphi(N):=(p-1)(q-1), t:=2^{T} \bmod \ordqrn, h_2:=g^t \bmod N$ and sets $\crs:=(N, T(\secpar), g, h_1, h_2)$. 
...
\begin{itemize}
\item If $i=j$ it runs the protocol with the honest verifier $\vrf$ with for statement $(h_1^j, h_2^j, c_0^j,c_1^j,c_2^j)$ using as a commitment value $a_0^j,a_1^j$. It obtains challenge $v$ from $\vrf$ and if $H(h_1^j, h_2^j,c_0^j,c_1^j,c_2^j,a_0^j,a_1^j)$ is set and not equal $v$, it aborts the experiment. Otherwise, it programs the oracle $H(h_1^j, h_2^j,c_0^j,c_1^j,c_2^j,a_0^j,a_1^j):=v$ if . Answer the query of $\adv$ with $v$.
...
\end{itemize}
\item Samples $b \rand \bits, x, y_2 \rand \qrn$ and computes $(c_0^*, c_1^*, c_2^*):=(x, x^{k_1} m_b,\allowbreak y_2 m_b)$. Run the HVZK simulator on $(h_1, h_2, c_0^*, c_1^*, c_2^*)$ to to obtain an accepting transcript $(a,v,z)$ and in case that $H(h_1, h_2, c_0^*, c_1^*, c_2^*,a)$ is already defined with value different to $v$ it aborts the experiment, otherwise it programs the oracle $H(h_1, h_2, c_0^*, c_1^*, c_2^*,a):=v$ and outputs $(a,z)$.
...
Suppose that the query $(h_1, h_2,c_0,c_1,c_2,a)$ has been asked to the random oracle as $i^*$-th query and that $i=i^*$. If the experiment is not aborted, then $\advB$ guesses a correct index for which $\adv$ asks decommitment query for a statement which is not in the specified language $L$. Therefore we get
\[\Pr[\event] \leq \snd^\nizk_\advB.\]
