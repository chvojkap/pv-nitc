...
\vspace{-2mm}
\begin{enumerate}
\item Set $\crs:=\mathlist(N, T(\secpar), g, h_1, h_2, \crs_\nizk)$, run $(m_0, m_1, \st) \leftarrow \adv_{1, \secpar}(\crs)$, and answer decommitment queries using $k_1$ (which is included in $\tau_L = (k_1, t)$.
...
\item Run $b' \leftarrow \adv_{2, \secpar}((c_0^*, c_1^*, c_2^*), \pi^*, \st)$, answering decommitment queries using $k_1$.
...
\]
\end{lemma}\todo{general proof for any n}
...
In $\games_\thisgame$ we answer decommitment queries using $\dec$ (\Cref{fig:deco-mh}) with $i:=2, \sk:=t$ which means that secret key $t$ and ciphertext $c_2$ are used. 
...
\item Samples $b \rand \bits, x, y_2\rand \qrn$ and computes $(c_0^*, c_1^*, c_2^*):=(x, x^{k_1} m_b,\allowbreak y_2 m_b)$. Forwards $(c_0^*, c_1^*, c_2^*)$ to simulation oracle $\simul_2$ and obtains a proof $\pi^*$.
