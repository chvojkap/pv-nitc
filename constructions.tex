...
$t:= 2^T \bmod \varphi(N)/2$ & $c := (c_0, c_1, c_2, c_3), w := (m, r)$ \\
For $i \in [2]: h_i:= g^{k_i} \bmod N$ &  $\pi_\com \leftarrow \nizk.\prove(\crs_\nizk, c, w)$\\
$h_3:=g^{t} \bmod N$ &  $\pi_\dec: = r$ \\
...
return $\nizk.\vrfy(\crs_\nizk, c, \pi_\com)$  & Parse $c$ as $(c_0, c_1, c_2, c_3)$ \\
 & if $ \land_{i=1}^3 c_i = h_i^{\pi_\dec N}(1+N)^m  \bmod N^2$ \\
...
\item It runs $b' \leftarrow \adv_{2, \secpar}(s, \pi^*, \st)$ and answers decommitment queries using $k_1$.
\item Finally, it checks whether there exists a decommitment query $(c, \pi_\com)$ such that $\deco(\crs, c, \pi_\com) \neq \dec(\crs,c, \pi_\com, 2)$. If $\event$ occurs, then this is the case, and it returns $(c, \pi_\com)$. Notice that this check can be done efficiently with the knowledge of $t$, since instead of running $\fdecom$, $\advB$ can verify the proof and compute $c_3 c_0^{-t} \bmod N$ which produces the same output as $\fdecom$.
\end{enumerate}
...
\item If there exists a decommitment query $(c, \pi_\com)$ such that $\dec(\crs, c, \pi_\com, 1) \neq \dec(\crs,c, \pi_\com, 2)$, then it returns $(c, \pi_\com)$. Note that such a query exists iff $\event$ happens.
\end{enumerate}
